...
def eval_ts_similarity(df, # df can be df_train / df_test
    ...
    #df, 包含时间序列 + 标签 + 文本的数据框,
    #y_col, 要编辑的属性（如'variability'）, InstructTime的“target
    #b = 200, 采样200
    #"aug_type=""conditional""", 条件式文本替换, InstructTime
    ...
    # if y_col=='holiday':
    #
    #     print("分组前holiday列的类别分布：")
    #     print(df['holiday'].value_counts(dropna=False))  # dropna=False显示缺失值数量
    #     print("\n分组前的唯一类别：", list(df['holiday'].unique()))
    ...
    for i in range(len(y_levels)):#遍历所有的原始属性类别
        # agument data with original reference text y_levels[i]
        ref_level = y_levels[i]     #属性i
        df_level = df[df[y_col] == ref_level].reset_index(drop=False).copy() #取 所有属于 ref_level 的样本
        # agument towards new text conditions 
        for j in range(len(y_levels)):  #遍历所有 目标类别（augmentation level）
            if not config_dict['open_vocab']:#每种属性的描述有很多种
                ...
                if aug_type == 'marginal':  #文本描述整个替换
                    ...
                    df_level[y_col+'_aug'] = np.random.choice(aug_level_map[y_levels[j]], size=len(df_level))#文本描述只替换目标段落
                    df_level['text' + str(j)]  = ''
                    for str_col in [col+'_aug' for col in config_dict['txt2ts_y_cols']]:
                        df_level['text' + str(j)] += ' ' + df_level[str_col]
                    df_level['text' + str(j)] = df_level['text' + str(j)].str.strip()       #为每个样本 随机选择一个目标表达，存入 y_col+'_aug'
        
        # mapping text_col to y_level
        col_level_map = dict(zip(['text' + str(j) for j in range(len(y_levels))], y_levels))        #指令列 → 目标标签”
    
        # only augment towards NEW text conditions except original aug_text
        new_text_cols = ['text' + str(j) for j in range(len(y_levels)) if y_levels[j] != ref_level]     #只编辑到“新”类别，跳过原始类别
        ...
        for text_col, pairs in ts_hat_ls.items():
            ...
            aug_ts_list = [aug_df['ts_hat'][i] for i in range(len(aug_df))]             #编辑后的时间序列
            
            # target time series
            tgt_df = df[df[y_col] == aug_level]
            tgt_ts_list = [tgt_df[[str(i+1) for i in range(config_dict['seq_length'])]].to_numpy()[i] for i in range(len(tgt_df))]          #真实目标类别的时间序列
            
            # orginal time series (reference time series) 
            ref_df = df[df[y_col] == ref_level]   
            ref_ts_list = [ref_df[[str(i+1) for i in range(config_dict['seq_length'])]].to_numpy()[i] for i in range(len(ref_df))]          #原始类别的时间序列
            ...
            _, _, _, _, dtw_aug2tgt, lcss_aug2tgt, mse_aug2tgt, mae_aug2tgt = calculate_similarity_parallel(aug_ts_list, tgt_ts_list, n_patches=n_patches)      #计算编辑后 vs 真实目标 的距离
            _, _, _, _, dtw_ref2tgt, lcss_ref2tgt, mse_ref2tgt, mae_ref2tgt = calculate_similarity_parallel(ref_ts_list, tgt_ts_list, n_patches=n_patches)      #计算原始 vs 真实目标 的距离
