...
class EvalCLIPTXT2TS:
    def __init__(self, 
        ...
        if not config_dict['3d']:
            self.ts_f_mat, self.tx_f_mat, _, self.var_f_mat = get_features(df_new,
                                                            config_dict,
                                                            text_col='caption')
            ...
            assert self.var_f_mat.shape[0] == self.n_captions * self.n_ts_candidates, "var_f_mat.shape[0] error"
...
def eval_clip_txt2ts(model, 
    ...
    with torch.no_grad(): 
        ...
        for cap_id in range(n_cap):
            ...
            var_f_mat = eval_inputs.var_f_mat[cap_id*n_ts:(cap_id+1)*n_ts,:]
            logits, _, _, _ = model(ts_f_mat, tx_f_mat, var_f_mat)
...
def eval_clip3d_txt2ts(model, 
    ...
    with torch.no_grad(): 
        ...
        for cap_id in range(n_cap):
            ...
            var_f_mat = eval_inputs.var_f_mat[cap_id*n_ts:(cap_id+1)*n_ts,:]
            logits, _, _, _ = model(ts_f_mat, tx_f_mat_ls, var_f_mat)
            logit = torch.diag(logits).unsqueeze(0) # (1, n_ts)
            exp_preds = torch.exp(logit)
            y_prob_row = exp_preds / exp_preds.sum(dim=1, keepdim=True)
            y_prob[cap_id,:] = y_prob_row
