...
class EvalCLIPTS2TXT:
    def __init__(self, 
        ...
        if not config_dict['3d']:
            self.ts_f_mat, self.tx_f_mat_ls, _, self.var_f_mat = get_features3d(df_new,
                                                                config_dict,
            ...
            self.var_f_mat = self.var_f_mat.to(device)
        
        elif config_dict['3d']:
            tx_f_mat_ls_ls = []
            for y_pred_cols in y_pred_cols_ls:
                # replace cl_event with text1 / text2 in tx_df_mat_ls
                ts_f_mat, tx_f_mat_ls, _, var_f_mat = get_features3d(df_new,
                                                          config_dict,
                                                          text_col_ls=y_pred_cols) # text1 is a column of "will die", text2 is a column of "will survive"
...
def eval_clip_ts2txt(model, 
              eval_inputs,
              return_probs=False,
              batch_size=512):  # Process logits in batches
    ...
    with torch.no_grad(): 
        ...
        var_f_mat = eval_inputs.var_f_mat
        ...
        for i in range(0, num_samples, batch_size):
            ...
            var_f_mat_batch = var_f_mat[i:batch_end]
            batch_logits = []
            for tx_f_mat in tx_f_mat_ls:
                logit, _, _, _ = model(ts_f_batch, tx_f_mat[i:batch_end], var_f_mat_batch)
    ...
    with torch.no_grad(): 
        ...
        for i in range(0, num_samples, batch_size):
            ...
            for tx_f_mat_ls in tx_f_mat_ls_ls:
                logits, _, _, _ = model(ts_f_batch, [tx_f_mat[i:batch_end] for tx_f_mat in tx_f_mat_ls], var_f_mat_batch)
