...
class TSClassifier(nn.Module):
    ...
    def __init__(self, 
        ...
        self.encoder = copy.deepcopy(emedit_model.ts_encoder).to(device).eval()
        if not finetune:
    ...
    def _prepare_df2train(self, df: pd.DataFrame, config_dict: dict, conditions: Optional[List[Tuple[str, str]]] = None, b: Optional[int] = None):
        ...
        if not config_dict['open_vocab']:
            df2train = df2train[
                self.ts_cols + config_dict["txt2ts_y_cols"] + ["text", "label","var"]
            ].copy()
        else:
            df2train = df2train[
    ...
    def predict(
        ...
        device: Union[str, torch.device] = "cpu",  # Python 3.7 支持的类型联合
    ) -> pd.DataFrame:
...
def prep_df2pred(
    ...
    model, # the emedit model object
    config_dict: dict, # the config of emedit model
    ...
    *,
    ...
    for tgt_level in y_levels:
        ...
        df2aug_src = df2aug[
            ts_str_cols + config_dict["txt2ts_y_cols"] + ["text", "label","var"]
        ].copy()
        ...
        if meta is not None:
            ...
            # add a 1e-8 perturbation to w
            w = float(w + 1e-8 * np.random.rand())
