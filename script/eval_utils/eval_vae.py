...
def plot_reconstructions(model,
    ...
    if config_dict['3d']:
        ts_f, tx_f_ls, _, var_f = get_features3d(df, config_dict, text_col_ls = config_dict['text_col_ls'])
        ts_f = ts_f.to(device)
        tx_f_ls = [tx_f.to(device) for tx_f in tx_f_ls]
        var_f = var_f.to(device)
        _, ts_hat, _, _ = model(ts_f, tx_f_ls, var_f)
    else:
        ts_f, tx_f, _, var_f = get_features(df, config_dict, text_col = 'text')        # [n,l] n是num_sample,从数据集采样n条数据，l是时间序列长度
        ts_f = ts_f.to(device)
        tx_f = tx_f.to(device)
        var_f = var_f.to(device)
        _, ts_hat, _, _ = model(ts_f, tx_f, var_f)             # [n,l]
...
def plot_reconstruction_from_distances(model, 
    ...
    if config_dict['3d']:
        ts_f, tx_f_ls, _, _ = get_features3d(df, config_dict, text_col_ls = config_dict['text_col_ls'])
        ts_f = ts_f.to(device)
        tx_f_ls = [tx_f.to(device) for tx_f in tx_f_ls]
        tx_emb, _ = model.text_encoder(tx_f_ls)
        x = ts_f[0]
    else:
        ts_f, tx_f, _, _ = get_features(df, config_dict, text_col = 'text')
        ts_f = ts_f.to(device)
        tx_f = tx_f.to(device)
