...
class emeditDataset(Dataset):
    def __init__(self, ts_features, text_features, labels, var_features, targets = None):
        """
        Args:
        ...
        if not isinstance(var_features, torch.Tensor):
            var_features = torch.FloatTensor(var_features)
        
        assert len(ts_features) == len(text_features) == len(labels)  == len(var_features), "All inputs must have the same length"
    ...
    def __getitem__(self, idx):
        return (
            ...
            self.labels[idx],
            self.var_features[idx],
            self.targets[idx]
        )
    def dataloader(self, batch_size=512):
...
class emedit3DDataset(Dataset):
    def __init__(self, ts_features, text_features_list, labels, targets = None):
        """
        Dataset for emedit3D model handling multiple text features per time series.
    ...
    def dataloader(self, batch_size=512, shuffle=True):
        """
...
def get_ts_txt_org(df, text_col = 'text', seq_length = 300):
    ...
    if 'emeditID' in df.columns:
        df = df.assign(id_time='id_' + df['emeditID'].astype(str) + '_' + df['emeditTime'].astype(str))
        df = df.set_index('id_time')
        df = df.drop(columns=['emeditID', 'emeditTime'])
    ...
    if 'var' in df.columns:
        txt_var_ls = df.loc[:,'var'].tolist()
    else:
        txt_var_ls = []
    return ts_df, txt_ls, labels, txt_var_ls


def get_features(df, 
                 config_dict,
                 text_col='text'):
    ts_df, txt_ls, labels, txt_var = get_ts_txt_org(df, text_col = text_col, seq_length = config_dict['seq_length'])
    # --- ts_f ---
    ...
    if 'level_maps' in config_dict.keys():
        tx_f = encode_attributes(df, config_dict['level_maps'], text_col)
        tx_var_f = []
    else:
        tx_f = TXTFeature(txt_ls, 
                        encoder_model_name=config_dict['text_encoder_name']).features
        tx_var_f = TXTFeature(txt_var,
                        encoder_model_name=config_dict['text_encoder_name']).features

    # --- labels ---
    labels = torch.tensor(labels)
    assert len(ts_f) == len(tx_f) == len(labels) == len(tx_var_f), "All inputs must have the same length"
    return ts_f, tx_f, labels, tx_var_f
...
def get_features3d(df, 
    ...
    ts_df, _, labels, tx_var = get_ts_txt_org(df, seq_length = config_dict['seq_length'])
    ts_f = TSFeature(ts_df, 
                     normalize_mean=config_dict['ts_normalize_mean'],
                     normalize_std=config_dict['ts_normalize_std'],
                     global_norm = config_dict['ts_global_normalize'], 
                     local_norm = config_dict['ts_local_normalize']).features
    tx_var_f = TXTFeature(tx_var,
                        encoder_model_name=config_dict['text_encoder_name']).features
    labels = torch.tensor(labels)
    tx_f_list = []
    for text_col in text_col_ls:
        _, txt_ls, _, _ = get_ts_txt_org(df, text_col = text_col, seq_length = config_dict['seq_length'])
        if 'level_maps' in config_dict.keys():
            tx_f = encode_attributes(df, config_dict['level_maps'], text_col)
        else:
            tx_f = TXTFeature(txt_ls, 
    ...
    return ts_f, tx_f_list, labels, tx_var_f
...
def encode_attributes(df, level_maps, text_col, onehot=True):
    ...
    level_maps : dict[str, dict[str,int]]
        {attr_col → {level_str → idx}}.  The order of keys defines the
        attribute order in the output.
    text_col : str
        Column whose free text may mention *some* of the attribute phrases.
        If a phrase for a given attribute appears, it overrides df[attr_col].
    onehot : bool, default True
        True  → return concatenated one-hot vectors.
        False → return raw integer indices (shape N × C).

    Returns
    -------
    torch.Tensor
        (N, ΣKi) if onehot else (N, C); dtype float32.
    """
    n_rows     = len(df)
    idx_cols   = []        # per-attribute integer indices
    n_classes  = []        # Ki for one-hot size
    text = df[text_col].astype(str)

    for col, level2idx in level_maps.items():
        # 1) baseline: use the value already in df[col]
        base_idx = df[col].map(level2idx).to_numpy(dtype=int)

        # 2) override where the free text mentions a level string
        for lvl_str, lvl_idx in level2idx.items():
            mask = text.str.contains(lvl_str, regex=False)
            base_idx[mask] = lvl_idx

        idx_cols.append(base_idx)
        n_classes.append(len(level2idx))

    idx_matrix = np.stack(idx_cols, axis=1)  # (N, C)

    if not onehot:                           # raw indices wanted
        return torch.tensor(idx_matrix, dtype=torch.float32)

    # one-hot per attribute, then concat
    onehot_parts = [
