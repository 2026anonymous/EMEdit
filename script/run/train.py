if overwrite:
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        ...
        mode='min',
        factor=0.9,         
        patience=10,    #config_dict['patience']
        min_lr=1e-20,        
        threshold=1e-4,      
        cooldown=20,
        verbose=True
    ...
    try:
        for i in range(config_dict['num_saves']):
            ...
            alpha_init = 0
            print(config_dict['num_saves'])
            # if i == config_dict['num_saves']-1:
            #     print('Train end ..........')
            #     break

            train_losses_tmp, test_losses_tmp, alpha_tmp = train_emedit(model,
                                                                        train_dataloader,
                                                                        test_dataloader,
                                                                        optimizer,
                                                                        scheduler,
        ...
        print(f"\tTraining error.  the error num is {i}...")
        raise e
