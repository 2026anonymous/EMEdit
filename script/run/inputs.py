if attr_suffix.startswith('_at_level'):
...
if overwrite:
    ...
    # ------------------------- ready dataloaders ------------------------- 
    ts_f_train, tx_f_train, labels_train, var_train = get_features(df_train, config_dict)
    if target_train is None: 
        target_train = gen_text_similarity_target(tx_f_train)
    train_dataloader = emeditDataset(ts_f_train, tx_f_train, labels_train, var_train, target_train).dataloader(batch_size=config_dict['batch_size'])
    ts_f_test, tx_f_test, labels_test, var_test = get_features(df_test, config_dict)
    if target_test is None: 
        target_test = gen_text_similarity_target(tx_f_test)
    test_dataloader = emeditDataset(ts_f_test, tx_f_test, labels_test, var_test, target_test).dataloader(batch_size=config_dict['batch_size'])
    ts_f_left, tx_f_left, labels_left, var_left = get_features(df_left, config_dict)
    if target_left is None: 
        target_left = gen_text_similarity_target(tx_f_left)
    left_dataloader = emeditDataset(ts_f_left, tx_f_left, labels_left, var_left, target_left).dataloader(batch_size=config_dict['batch_size'])
        
# ------------------------- ready input features dimension -------------------------
# get the dimension of input features
if 'ts_f_dim' not in locals():
    ts_f_dim, tx_f_dim, labels_dim, var_dim = get_features(df_train.iloc[:1,:], config_dict)
