...
def interpolate_ts_tx(df, model, config_dict, text_cols, w):
    ...
    _, tx_f_src, _, _ = get_features(df, config_dict, text_col = 'text')
    ...
    ts_hat_ls = {}
    for txid in range(len(tx_f_ls)):        #遍历每个 目标文本条件
        ts_hat, _, _, _ = model.generate(w, ts_f, tx_f_ls[txid], tx_f_src, var_f)  #编辑后的 time series
...
def interpolate_ts_tx_single(df, model, config_dict, text_cols, w, label = False):
    ...
    for txid in range(len(tx_f_ls)):
        ts_hat, ts_emb_tgt, tx_emb_tgt, _ = model.generate(w, ts_f, tx_f_ls[txid], tx_f_src, var_f)
        # # plot the ts_hat
...
def interpolate_ts_tx_single_sampling(df, model, config_dict, text_cols, w, 
    ...
    for txid in range(len(tx_f_ls)):
        ...
        ts_hat, _, _, _ = model.generate(w, ts_f, tx_f_tgt, tx_f_src, var_f)
